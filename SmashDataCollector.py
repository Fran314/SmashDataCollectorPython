...
for match_index in range(tot_matches):
    ...
    try:
        ...
        for player_index in range(players):
            ...
            curr_place = numpy.argmin([numpy.linalg.norm(curr_place_colour - colour) for colour in res.PLACE_COLOURS])+1
            ...
            curr_character_index = fun.getClosestCharacterByCloseup(curr_character_image, res.CHARACTER_CLOSEUPS[players-2], res.CHARACTER_CLOSEUPS_MASKS[players-2][1 if curr_place == 1 else 0], res.BACKGROUND_CLOSEUPS_BGRS[player_index])
        ...
        for player_index in range(players):
            ...
            x_start = res.LEFT_EDGE[players-2][player_index] + res.ANCHOR_POINT_LD[players-2]
            matching = cv2.matchTemplate(second_data[:, x_start : x_start + stencil_width], res.ANCHOR_POINT_STENCILS[player_index], cv2.TM_SQDIFF)
            # TODO eeeeeeeeeh this might actually not work, we'll see
            anchor_point = numpy.argmin(matching)
            ...
            for j in range(custom.LIVES):
                ...
                killer = fun.getClosestPlayerByIcon(fall_image, [res.CHARACTER_ICONS[index] for index in character_indices], res.BACKGROUND_ICONS_BGRS[player_index])
                if(killer == -1):
                    break
        ...
        output_strings.append(fun.convertMatchToString(dirs[2*match_index], players, [res.CHARACTER_INFOS[index][0] for index in character_indices], places, times, falls, given_damages, taken_damages))
